...
function ProductDetailsContent() {
  ...
  return (
    <div className="min-h-screen bg-[#e3e6e6] flex flex-col font-sans mb-16 md:mb-0">
                ...
                <button 
                  onClick={async (e) => {
                    ...
                    if (copied) {
                      ...
                      showToast("Failed to copy link. Please copy it from the address bar.", "info");
                    }
                  }}
      ...
      {isLightboxOpen && (
        <div 
          className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/95 backdrop-blur-md animate-fade-in"
        ...
        >
          ...
          <button 
            ...
            className="absolute top-6 right-6 md:top-8 md:right-8 z-[110] p-3 rounded-full bg-white/10 hover:bg-white/25 text-white transition-all cursor-pointer hover:scale-105 active:scale-95 border border-white/20"
            title="Close (Esc)"
          >
            <X size={24} />
          </button>
          ...
          <div className="w-full flex-grow flex items-center justify-between px-4 md:px-12 max-w-6xl relative z-50">
            <button 
              onClick={(e) => {
                e.stopPropagation();
                setLightboxImageIndex(prev => (prev === 0 ? galleryImages.length - 1 : prev - 1));
              }}
              className="p-3 md:p-4 rounded-full bg-white/10 hover:bg-white/25 text-white transition-all cursor-pointer active:scale-95 hover:scale-105"
            ...
            <div 
              className="flex-1 h-[70vh] flex items-center justify-center p-2 md:p-4 cursor-default"
              onClick={(e) => e.stopPropagation()}
            >
              <img 
                src={galleryImages[lightboxImageIndex]} 
                alt="" 
                className="max-h-[70vh] max-w-full object-contain rounded-2xl select-none animate-scale-in bg-white p-2 md:p-4 shadow-2xl" 
              />
            </div>
          ...
          <div 
            className="pb-8 flex gap-3.5 px-4 overflow-x-auto max-w-full z-50"
            onClick={(e) => e.stopPropagation()}
          >
            {galleryImages.map((img, idx) => (
              <div 
