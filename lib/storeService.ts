...
export interface Carousel {
  ...
  source_type?: string; // "MANUAL", "BEST_SELLERS", "OFFERS", "CATEGORY"
  source_value?: string;
}
...
const normalizeDeliverySlab = (slab: any): DeliverySlab => ({
  ...
  max_qty: slab.max_qty === null || slab.max_qty === undefined || slab.max_qty === ""
    ? null
...
const normalizeDeliverySlabs = (slabs: any): DeliverySlab[] => {
  ...
  return slabs
    ...
    .filter(slab => Number.isFinite(slab.min_qty) && slab.min_qty > 0 && Number.isFinite(slab.charge));
};
